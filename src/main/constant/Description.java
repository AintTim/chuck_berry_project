package main.constant;

public class Description {
    public static String ENCRYPT_DESC = "Шифровка текста";
    public static String DECRYPT_DESC = "Расшифровка текста с помощью ключа";
    public static String BRUTE_FORCE_DESC = "Расшифровка текста с помощью brute force";
    public static String STATISTICS_ANALYSIS_DESC = "Расшифровка текста с помощью статистического анализа";
    public static String EXIT_DESC = "Прекратить работу";

    private Description() {}
}
